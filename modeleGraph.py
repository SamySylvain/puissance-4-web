...
def evaluer_plateau(plateau, jeton_ia, nbrLignes, nbrColonnes):
    ...
    # ----------------------------------------------------------------
    # Bonus double menace réelle
    # Si un joueur a ≥2 menaces actives en même temps, l'adversaire ne
    # peut en bloquer qu'une → victoire quasi-forcée.
    # ----------------------------------------------------------------
    m_ia  = _menaces_reelles(plateau, jeton_ia, nbrLignes, nbrColonnes)
    m_adv = _menaces_reelles(plateau, adv,      nbrLignes, nbrColonnes)

    if m_ia  >= 2: score += 40000 + 10000 * (m_ia  - 2)
    if m_adv >= 2: score -= 40000 + 10000 * (m_adv - 2)
